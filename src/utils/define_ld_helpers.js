...
async function defineLD(sourceDefineJson, contextRef) {
    if (!contextRef) contextRef = 'http://localhost:4000/define_spec_context#'

    x = require(sourceDefineJson)
    ...
    fs.writeFileSync("output_expanded_define.json", JSON.stringify(expanded, null, 2))
    ...
    compacted['getDecodeByTerm'] = function (cl, term) {
        try {
    ...
    fs.writeFileSync("output_compacted_define.json", JSON.stringify(compacted, null, 2))
    return compacted
}
...
module.exports = { defineXML2defineLD, defineLD }